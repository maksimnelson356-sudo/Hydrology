...
> Дальше — **P1 / HydroSphere 1.5**: этапы P1.1–P1.7 ниже; **P1.1–P1.5 и P1.7 выполнены и запушены** (HEAD `54a7257`), мёрдж в `main` выполнен (origin/main = `54a7257`); открыт P1.6.
...
| P1.6 | GIS/DEM / морфометрия | `geo_service.py` (или min ввод) | ~3–5 дней (зависит от решения 9.3) |
...
| 8.2 | Ветка и стратегия коммитов | **(а) `global-implementation`**, затем мёрдж в `main` при готовности. **Мёрдж выполнен 23.09.2026** (fast-forward `d3967c7` → `54a7257`, origin/main = `54a7257`) | Этапы 0–7 |
...
|------|--------|-----------|
| 23.09.2026 | 2.0 | **P1.1–P1.5 + P1.7 выполнены и запушены** (`origin/global-implementation`, HEAD `54a7257`); **мёрдж в `main` выполнен** (решение 8.2, fast-forward `d3967c7` → `54a7257`, origin/main = `54a7257`). Тесты: 217 pytest + 135 корневых; навигация 23/23/23; GUI smoke OK; 20 методик в реестре. P1.7: `scenario_type`, `ReservoirScenarioService`, `storage_yield`, GUI-блок водохранилища + Δ-таблица + баланс-график. Открыт P1.6 (решение 9.3) |
