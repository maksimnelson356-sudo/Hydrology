...
>
> **Статус:** ТЗ, версия 3.0 · **Дата:** 24.09.2026 · **Ветка:** `global-implementation`
> **P0 (этапы 0–7), P1, P2 и P3.1–P3.4 выполнены и запушены** (`origin/global-implementation`, P3.4 release tip `4a7f018`).
...
> **§6.3 P3**: P3.1–P3.4 выполнены и запушены (`backwater_profile@1.0`, `muskingum@1.0`, `inundation@1.0`, `hydraulic_uncertainty@1.0`); P3.5 реализован (`906f002`..`388cb06`); решения 11.1=(а)/11.2=(а)/11.3=(а) закрыты.
...
| **HydroSphere 3.0** | P3 | 1D/2D гидравлическое моделирование, пространственные расчёты, интеграция ГИС, крупные инженерные модели | Этапы P3.1–P3.5 (см. §6.3) |
...
| P2.3 | Климатические сценарии (delta-change) | **готово** `climate_service.py` + GUI-блок до/после | ~2–3 дня |
| P2.4 | Расширенная визуализация | **готово** `plot_style` helpers (fan/hist/tornado) | ~1–2 дня |
| P2.5 | Decision Support | **готово** `decision_support_service.py` (10.3=а); report-секция не добавлялась (13 не тронуты) | ~2–3 дня |
...
## 6.3. Этапы P3 (HydroSphere 3.0) с критериями приёмки

Контекст: ядро уже содержит точечные 1D-инструменты (`backwater.py` — direct step
на трапецеидальном сечении, `flood_hydrograph.py`, `spillway.py`, `profile.py` —
живое сечение/отсеки, `geo_service.py` — GeoJSON-морфометрия). P3 собирает из них
**пространственные/цепочные** расчёты и пропагацию во времени, не переписывая ядро.
Новых runtime-зависимостей нет (решения 10.x, стек: numpy/scipy/pandas/PyQt6).

### Этап P3.1. Многопролётная кривая подпора (1D along-river)

**Цель:** профиль реки как **цепочка пролётов** с разной геометрией (B, m, n, I, L)
→ одна линия уровня воды от водохранилища/нижней границы вверх по течению.

**Задачи:**

1. `core/hydraulics.py` (или новый модуль рядом, не в сервисах): `Reach` + `route_backwater_profile(reaches, Q, H_downstream)` —
   вызов существующего `backwater_curve_step` на каждом пролёте, стыковка глубин на границах.
2. `core/services/backwater_profile_service.py` — тонкий адаптер + validation +
   provenance `backwater_profile@1.0`; выход JSON-safe.
3. Handler в реестре методик (опционально, если встраивается в CalculationService без ломки 20 методик).
4. GUI: расширение `widget_work9` (вкладка «Кривые подпора») — таблица пролётов + общий график WSE.
5. Тесты: 1 пролёт ≡ `backwater_curve_step`; стыковка (h_end[i] ≈ h_start[i+1]); ошибки
   (пустой список, L≤0, отрицательный Q).

**Критерии приёмки:**

- На одном пролёте числа бит-в-бит совпадают с `backwater_curve_step`.
- Новых зависимостей нет; ядро `backwater.py` не изменяется (только вызывается).
- pytest зелёные; nav/smoke не падают (GUI — внутри work9, не новый nav-раздел).

**Как проверяем:** pytest + ручной прогон в work9.

---

### Этап P3.2. Маршрутизация паводка (1D unsteady, Muskingum)

**Цель:** пропуск гидрографа вдоль русла — сглаживание и запаздывание пика на выходе
(без Saint-Venant; Muskingum — отраслевой стандарт для инженерных задач).

**Статус:** выполнен 24.09.2026; решение 11.1 = **(а) Muskingum**, provenance `muskingum@1.0`.

**Задачи:**

1. `core/hydrorash/routing.py` — `muskingum_route(inflow, dt, K, x, outflow0)`:
   C0/C1/C2 по классической схеме; проверка устойчивости `0 ≤ C0,C1,C2 ≤ 1`, `K ≥ dt`, `0 ≤ x ≤ 0.5`.
2. `core/services/routing_service.py` — `RoutingRequest`/`RoutingResult`
   (pik attenuations, lag, volume conservation), provenance `muskingum@1.0`.
3. GUI: блок в `tab_monte_carlo` или work7 (гидрографы) — входной гидрограф → выходной, график до/после.
4. Тесты: объём сохраняется (ΣQ·dt); постоянный вход → установившийся выход = вход;
   пики: аттенюация ≥ 0, запаздывание ≥ 0; ошибки (x вне [0,0.5], K < dt, пустой ряд).

**Критерии приёмки:**

- Объём (интеграл) на выходе ≈ входу (допуск 1% на дискретизации).
- Постояный Q → стационарный выход без осцилляций.
- Новых зависимостей нет.

**Как проверяем:** pytest (аналитика + conservation) + GUI-сравнение двух гидрографов.

---

### Этап P3.3. Затопление: уровень → площадь/объём (1D-2D bridge)

**Цель:** из линии уровня (P3.1) и морфометрии/контуров (P1.6) — оценка площади
затопления и объёма воды «под кривой»; без растрового DEM (решение 9.3=(б)).

**Статус:** выполнен 24.09.2026; решение 11.2 = **(а) S(H)/таблица**, GeoJSON-контуры — совместимый второй источник, provenance `inundation@1.0`.

**Задачи:**

1. `core/services/inundation_service.py` — по (H, series of stage-area points | profile
   geometry at h | GeoJSON contour elevations): `inundation_from_stage(...)`.
   Источники площади: (а) кривая S(H) из `profile.build_curves` / таблицы;
   (б) полигоны GeoJSON с `elevation` в properties (если есть) — фильтрация по H.
2. Provenance `inundation@1.0`; ошибки (нет точек S(H), H вне диапазона → warning, не crash).
3. GUI: блок рядом с гео/подпором — ввод H → площадь, объём, доля от поймы.
4. Тесты: монотонность S(H); аналитическая трапецеида; пустые входы; roundtrip JSON.

**Критерии приёмки:**

- S(H) монотонно не убывает; на прямоугольном сечении площадь = B·H.
- Без растров и новых библиотек; GeoJSON — только properties elevation (не geometry ops).

**Как проверяем:** pytest + ручной прогон H → S,V в GUI.

---

### Этап P3.4. Пропагация неопределённостей через гидравлику (MC × P3)
...
### Этап P3.5. Экспорт инженерных моделей (HEC-RAS-like / JSON)

**Статус:** реализован 24.09.2026; решение 11.3 = (а) — собственный JSON + CSV
в `utf-8-sig`. Code/build commits: `906f002`, `7974a8b`, `5b281e4`, `8f70a87`, `5090fec`, `388cb06`.

**Цель:** выгрузка пролётов, гидрографов и WSE в обменные форматы для внешних
инструментов (и обратный импорт простых JSON-моделей) — «крупные инженерные модели».

**Задачи:**

1. `core/services/model_export_service.py` — `export_reaches` / `export_hydrograph` /
   `export_wse_profile` → CSV + JSON (utf-8-sig, как отчёт); `import_json_model` (tolerant).
2. GUI: кнопки «Экспорт…» в work9 / routing-блоке; файл рядом с `.hsp` или через QFileDialog.
3. Тесты: roundtrip JSON; CSV читается pandas; ошибки (нет данных, битый JSON → понятное сообщение).

**Критерии приёмки:**

- Roundtrip: export → import → те же reaches/Q series (допуск float formatting).
- Новых зависимостей нет; не ломает `.hsp` (формат отдельный).

**Как проверяем:** pytest + ручной экспорт/импорт в GUI.
...
| P3.5 | Экспорт инженерных моделей — **реализовано** | `model_export_service`, CSV/JSON, Work7/Work9 export | ~2 дня |

**Порядок (рекомендуемый):** P3.1 → P3.2 → P3.3 → P3.4 → P3.5.
Профиль → маршрутизация → затопление → пропагация MC → экспорты.
...
| 10.3 | Decision support: пороги риска | **(а) пользовательские пороги Q_крит** (закрыто 23.09.2026 при P2.5): инженер задаёт явный Q_крит; перцентили из MC — только справочно | P2.5 |

### Открытые решения P3 (перед соответствующим этапом)

| # | Вопрос | Варианты | Затронуло |
|---|--------|----------|-----------|
| 11.1 | Алгоритм маршрутизации 1D | **(а) Muskingum** (закрыто 24.09.2026 при P3.2; `muskingum@1.0`) · (б) полноценный Saint-Venant отклонён как тяжёлый и требующий новых зависимостей | P3.2 |
| 11.2 | Источник S(H) для затопления | **(а) кривая/таблица S(H)** (закрыто 24.09.2026 при P3.3; `inundation@1.0`) · **(б) GeoJSON `properties.elevation`** поддержан как второй источник · **(в) DEM-растер** запрещён решением 9.3=б | P3.3 |
| 11.3 | Формат экспорта инженерной модели | **(а) свой JSON + CSV (закрыто 24.09.2026 при P3.5; `utf-8-sig`, tolerant JSON import)** · (б) HEC-RAS-like `.g01/.d01` отложен до появления конкретного внешнего потребителя | P3.5 |
...
| 23.09.2026 | 2.5 | **§6.3 Этапы P3 (P3.1–P3.5) расписаны**: многопролётная кривая подпора, Muskingum-маршрутизация, затопление H→S,V, MC×гидравлика, экспорт инженерных моделей; добавлены открытые решения 11.1–11.3; шапка/таблица версионирования → v2.5; код P3 не начат |
| 23.09.2026 | 2.4 | **P2.3–P2.5 выполнены; §6.2 P2 закрыт**: `climate_service` (×(1+δ)/+δ, metadata `.hsp`, 17 тестов), P2.4 helpers (`draw_fan_chart` / `draw_histogram_quantiles` / `draw_tornado_hbars`) + подключение в `tab_monte_carlo`, `decision_support_service` (P(exceed), класс риска, **решение 10.3 (а)** закрыто, 23 теста); GUI-блоки «Климат» и «Решения»; **313 passed** + 135 root; report 13 секций не тронуты |
